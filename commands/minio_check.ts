...
export default class MinioCheck extends BaseCommand {
  ...
  async run() {
    ...
    this.logger.info(`  - Access Key: ${minioConfig.accessKey ? '***' + minioConfig.accessKey.slice(-4) : 'NON DÉFINIE'}`)
    this.logger.info(`  - Secret Key: ${minioConfig.secretKey ? '***' + minioConfig.secretKey.slice(-4) : 'NON DÉFINIE'}`)
